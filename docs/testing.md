...
**Properties set on a `SpawnActorAt<T>(...)` return value can be too late for that actor's own `BeginPlay()`.** A `FMapTestSpawner` world has already begun play, so `World.SpawnActor<T>()` runs `BeginPlay()` synchronously inside the call, before the line after it (`Zone.DamagePerSecond = 100.f;` etc.) executes. Any value a native actor's `BeginPlay()` reads once and captures (e.g. `AALSDamageZone`'s `SetTimer(..., DamageIntervalSeconds, ...)`, which locks in whatever `DamageIntervalSeconds` was at that instant) can't be tuned this way - it'll silently use the class default instead. Either accept the defaults in the test, or use deferred spawning (`SpawnActorDeferred`/`FinishSpawning`) to set properties before `BeginPlay` runs.

**`FMapTestSpawner`'s temp level has no floor.** A character spawned there free-falls under normal gravity indefinitely - confirmed by logging actor location: Z=0 -> Z=-294 in 0.77s, matching unobstructed free-fall. Pickup tests (`ALSPickupTests.cpp`) get away with this because their overlap check happens synchronously, at the moment of a `SetActorLocation(..., /*bSweep=*/true)` call, before gravity has accumulated any meaningful drop - not because a floor exists. Any test that needs an actor to stay in one place for longer than a fraction of a second (e.g. `ALSDamageZoneTests.cpp`'s `StandingInZone_TakesDamageOverTime`, which checks a periodic-damage timer that only ticks once per second) must explicitly `Character->GetCharacterMovement()->DisableMovement()` right after spawning - don't assume standing still is free.
